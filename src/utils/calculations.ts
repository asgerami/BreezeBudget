...
// calculate BTU requirement for AC unit
// based on some industry standards i found online
...
export const generateCostProjections = async (
  ...
  if (weatherData.lat && weatherData.lon) {
    try {
      monthlyTemperatures = await fetchHistoricalWeather(
        weatherData.lat,
        weatherData.lon
      );
    } catch (error) {
  ...
  const electricityRate = weatherData.state
    ? await fetchElectricityRates(weatherData.state)
    : 0.13;
