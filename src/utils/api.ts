...
// get electricity rates by state
// TODO: use real API instead of hardcoded values
...
export const fetchHistoricalWeather = async (
  ...
  try {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      const average =
        monthTemps.reduce((sum: number, temp: number) => sum + temp, 0) /
        monthTemps.length;
...
export const fetchDetailedWeatherData = async (
  lat: number,
  lon: number
...
export const fetchWeatherData = async (
  zipCode: string
): Promise<WeatherData & { lat: number; lon: number; state: string }> => {
  try {
